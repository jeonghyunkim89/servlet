...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * ---------
		 * 
		 * 요청 시 전달된 데이터 추출 ( request 객체의 parameter영역 안에 데이터가 담겨있음)
		 * request.getParameter("키")
		 */
		String userName = request.getParameter("userName"); //"홍길동"|""
		String gender = request.getParameter("gender");		//"남자"|"여자"|null (전달된 값이 없을때)
		int age =Integer.parseInt(request.getParameter("age"));	//"20" -> 20 | ""
		String address = request.getParameter("address");
		double height = Double.parseDouble(request.getParameter("height")); //"160" -> 160.00000
		...
		//같은 키값으로 여러개의 데이터가 전달되는 경우 : getParameterValues("키") : String[]
